	...
	for(int i = AppCmdCtrl_TopicType_SingleDevice; i < AppCmdCtrl_TopicType_Max; i++) {
		...
		switch(i) {
			case AppCmdCtrl_TopicType_SingleDevice: {
				appCmdCtrl_SubscribeInfosCommandsArray[i].topic = AppMisc_FormatTopic("%s/iot-control/%s/device/%s/command",
																	methodCreate, baseTopic, appCmdCtrl_DeviceId);
				appCmdCtrl_SubscribeInfosConfigurationsArray[i].topic = AppMisc_FormatTopic("%s/iot-control/%s/device/%s/configuration",
																	methodUpdate, baseTopic, appCmdCtrl_DeviceId);
			} break;
			case AppCmdCtrl_TopicType_Level3: {
				appCmdCtrl_SubscribeInfosCommandsArray[i].topic = AppMisc_FormatTopic("%s/iot-control/%s/device/command",
																	methodCreate, baseTopic, NULL);
				appCmdCtrl_SubscribeInfosConfigurationsArray[i].topic = AppMisc_FormatTopic("%s/iot-control/%s/device/configuration",
																	methodUpdate, baseTopic, NULL);
			} break;
			case AppCmdCtrl_TopicType_Level2: {
				appCmdCtrl_SubscribeInfosCommandsArray[i].topic = AppMisc_FormatTopic("%s/iot-control/%s/%s/device/command",
																	methodCreate, baseTopicLevelsArray[0], baseTopicLevelsArray[1]);
				appCmdCtrl_SubscribeInfosConfigurationsArray[i].topic = AppMisc_FormatTopic("%s/iot-control/%s/%s/device/configuration",
																	methodUpdate, baseTopicLevelsArray[0], baseTopicLevelsArray[1]);

			} break;
			case AppCmdCtrl_TopicType_Level1: {
				appCmdCtrl_SubscribeInfosCommandsArray[i].topic = AppMisc_FormatTopic("%s/iot-control/%s/device/command",
																	methodCreate, baseTopicLevelsArray[0], NULL);
				appCmdCtrl_SubscribeInfosConfigurationsArray[i].topic = AppMisc_FormatTopic("%s/iot-control/%s/device/configuration",
																	methodUpdate, baseTopicLevelsArray[0], NULL);
			} break;
			default: assert(0);
		}
	} // for
