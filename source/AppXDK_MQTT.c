...
Retcode_T AppXDK_MQTT_PublishToTopic(const AppXDK_MQTT_Publish_T * publishPtr) {
	...
	const char *solaceTopic = "BoschBrgP/P54/xdk/telemetry";
	assert(publishPtr);
	...
	case AppXDK_MQTT_TypeServalStack: {
		...
		//StringDescr_wrap(&publishTopicDescription, solaceTopic);
        ...
        if (pdTRUE != xSemaphoreTake(appXDK_MQTT_PublishSemaphoreHandle, 0UL)) {
    		...
    		appXDK_MQTT_PublishStatus = false;
    																																								//Retained = true
    		if (RC_OK != Mqtt_publish(&appXDK_MQTT_ServalSession, publishTopicDescription, publishPtr->payload, publishPtr->payloadLength, (uint8_t) publishPtr->qos, true)) {
				#ifdef DEBUG_APP_XDK_MQTT
