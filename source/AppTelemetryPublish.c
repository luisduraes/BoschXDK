...
Retcode_T AppTelemetryPublish_Setup(const AppRuntimeConfig_T  * configPtr) {
	...
	if(RETCODE_OK == retcode) retcode = AppTelemetryPublish_ApplyNewRuntimeConfig(AppRuntimeConfig_Element_targetTelemetryConfig,
											configPtr->targetTelemetryConfigPtr);

	if(RETCODE_OK == retcode) retcode = AppTelemetryPublish_ApplyNewRuntimeConfig(AppRuntimeConfig_Element_topicConfig,
											configPtr->topicConfigPtr);

	if(RETCODE_OK == retcode) retcode = AppTelemetryPublish_ApplyNewRuntimeConfig(AppRuntimeConfig_Element_activeTelemetryRTParams,
...
Retcode_T AppTelemetryPublish_ApplyNewRuntimeConfig(AppRuntimeConfig_ConfigElement_T configElement, const void * newConfigPtr) {
	...
	char *solaceTelemetryTopic = "BoschBrgP/P54/xdk/telemetry";
	...
	if(pdTRUE == xSemaphoreTake(appTelemetryPublish_TaskSemaphoreHandle, APP_TELEMETRY_PUBLISHING_TASK_DELETE_INTERNAL_WAIT_TICKS)) {
		...
		case AppRuntimeConfig_Element_topicConfig: {
			...
			appTelemetryPublish_MqttPublishInfo.topic = solaceTelemetryTopic;
