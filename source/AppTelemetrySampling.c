 ...
 */
Retcode_T AppTelemetrySampling_Init(const char * deviceId, uint32_t samplingTaskPriority,
		uint32_t samplingTaskStackSize, const CmdProcessor_T * sensorProcessorHandle) {
	...
	if(appTelemetrySampling_TaskSemaphoreHandle == NULL)
		return RETCODE(RETCODE_SEVERITY_FATAL, RETCODE_SOLAPP_FAILED_TO_CREATE_SEMAPHORE);
...
Retcode_T AppTelemetrySampling_Setup(const AppRuntimeConfig_T * configPtr) {
	...
	if (RETCODE_OK == retcode) retcode = AppTelemetrySampling_ApplyNewRuntimeConfig(AppRuntimeConfig_Element_activeTelemetryRTParams,
			configPtr->activeTelemetryRTParamsPtr);
 ...
 */
Retcode_T AppTelemetrySampling_ApplyNewRuntimeConfig(AppRuntimeConfig_ConfigElement_T configElement,
		const void * newConfigPtr) {
	...
	case AppRuntimeConfig_Element_activeTelemetryRTParams:
		...
		appTelemetrySampling_SamplingPeriodicityMillis =
				((AppRuntimeConfig_TelemetryRTParams_T *) newConfigPtr)->samplingPeriodicityMillis;
